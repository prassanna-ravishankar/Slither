...
namespace Slither
{
  ...
  class PatchLinearFeatureSVMFactory: public IFeatureResponseFactory<PatchLinearFeatureResponseSVM>
  {
  public:
      PatchLinearFeatureResponseSVM CreateRandom(Random& random, const IDataPointCollection& data, unsigned int* dataIndices, const unsigned int i0, const unsigned int i1,float svm_c, FeatureMaskType featureMask, bool root_node);
  };
  ...
  public:
    ...
    template<class F>
    const PixelBgr TraditionalClassification<F>::UnlabelledDataPointColor = PixelBgr::FromArgb(192, 192, 192);


    template<class F>
    class PatchClassificationTrainingContext : public ITrainingContext<F,PatchHistogramAggregator> // where F:IFeatureResponse
    {
    private:
        int nClasses_;
    ...
    public:
        IGType igType;
        PatchClassificationTrainingContext(int nClasses, IFeatureResponseFactory<F>* featureFactory)
        {
          nClasses_ = nClasses;
          featureFactory_ = featureFactory;
        }
    ...
    private:
        ...
        PatchHistogramAggregator GetStatisticsAggregator()
        {
          return PatchHistogramAggregator(nClasses_);
        }

        double ComputeInformationGain(const PatchHistogramAggregator& allStatistics, const PatchHistogramAggregator& leftStatistics, const PatchHistogramAggregator& rightStatistics)
        {
          //std::cout<<"Selected IG : "<<igType<<std::endl;
          switch(igType)
          {
        ...
        double ComputeInformationGainShannon(const PatchHistogramAggregator& allStatistics, const PatchHistogramAggregator& leftStatistics, const PatchHistogramAggregator& rightStatistics)
        {
          unsigned int nTotalSamples = leftStatistics.SampleCount() + rightStatistics.SampleCount();
          if (nTotalSamples <= 1)
            return 0.0;
        ...
        double ComputeInformationGainGINI(const PatchHistogramAggregator& allStatistics, const PatchHistogramAggregator& leftStatistics, const PatchHistogramAggregator& rightStatistics)
        {
          unsigned int nTotalSamples = leftStatistics.SampleCount() + rightStatistics.SampleCount();
          if (nTotalSamples <= 1)
            return 0.0;
        ...
        double ComputeInformationGainReweighted(const PatchHistogramAggregator& global, const PatchHistogramAggregator& allStatistics, const PatchHistogramAggregator& leftStatistics, const PatchHistogramAggregator& rightStatistics)
        {
          // TODO: Bad!!!!
          unsigned int nTotalSamples = leftStatistics.SampleCount() + rightStatistics.SampleCount();
          if (nTotalSamples <= 1)
        ...
        bool ShouldTerminate(const PatchHistogramAggregator& parent, const PatchHistogramAggregator& leftChild, const PatchHistogramAggregator& rightChild, double gain)
        {
          return gain < 0.01;
        }
    };
    ...
    template<class F>
    class PatchwiseClassification
    {
        static const PixelBgr UnlabelledDataPointColor;

    public:
        static std::unique_ptr<Forest<F, PatchHistogramAggregator> > Train (
                const ImageDataPointCollection& trainingData,
                IFeatureResponseFactory<F>* featureFactory,
                const TrainingParameters& trainingParameters ) // where F : IFeatureResponse
        {
          if (trainingData.HasLabels() == false)
          ...
          PatchClassificationTrainingContext<F> classificationContext(trainingData.CountClasses(), featureFactory);
          classificationContext.igType = trainingParameters.igType;


          std::unique_ptr<Forest<F, PatchHistogramAggregator> > forest
                  = ParallelForestTrainer<F, PatchHistogramAggregator>::TrainForest (
                          random, trainingParameters, classificationContext, trainingData );
        ...
        static std::unique_ptr<Forest<F, PatchHistogramAggregator> > TrainSingle (
                const ImageDataPointCollection& trainingData,
                IFeatureResponseFactory<F>* featureFactory,
                const TrainingParameters& trainingParameters ) // where F : IFeatureResponse
        {
          ...
          std::unique_ptr<Forest<F, PatchHistogramAggregator> > forest
                  = ForestTrainer<F, PatchHistogramAggregator>::TrainForest (
                          random, trainingParameters, classificationContext, trainingData );
        ...
        static std::unique_ptr<Forest<F, PatchHistogramAggregator> > TrainParallel (
                const ImageDataPointCollection& trainingData,
                IFeatureResponseFactory<F>* featureFactory,
                const TrainingParameters& trainingParameters ) // where F : IFeatureResponse
        {
          ...
          std::unique_ptr<Forest<F, PatchHistogramAggregator> > forest
                  = ForestTrainer<F, PatchHistogramAggregator>::TrainForestParallel (
                          random, trainingParameters, classificationContext, trainingData );
        ...
        /// <param name="testData">Test data</param>
        /// <returns>An array of class distributions, one per test data point</returns>
        static void Test(const Forest<F, HistogramAggregator>& forest, const ImageDataPointCollection& testData, std::vector<PatchHistogramAggregator>& distributions) // where F : IFeatureResponse
        {
          int nClasses = forest.GetTree(0).GetNode(0).TrainingDataStatistics.BinCount();
          ...
          std::vector<PatchHistogramAggregator> result(testData.Count());
          int correctCount = 0;

          for (int i = 0; i < testData.Count(); i++)
          {
            // Aggregate statistics for this sample over all leaf nodes reached
            result[i] = PatchHistogramAggregator(nClasses);
            for (int t = 0; t < forest.TreeCount(); t++)
            {
              int leafIndex = leafIndicesPerTree[t][i];
              result[i].Aggregate(forest.GetTree(t).GetNode(leafIndex).TrainingDataStatistics);
