...
#define WITH_FISHER true
  ...
  public:
    ...
    class LinearFeatureResponsePatchesSVM
    {
        friend class boost::serialization::access;
    protected:
        std::vector<int> vIndex_;
    ...
    public:
        LinearFeatureResponsePatchesSVM():
                dimensions_(-1),
                bias_(0.0f)
        {
            dimensions_ = 10;
        ...
        /// <param name="dx">The first element of the direction vector.</param>
        /// <param name="dx">The second element of the direction vector.</param>
        LinearFeatureResponsePatchesSVM(float* pWeights, const int dimensions)
        {

            //vWeights_ = std::vector<float>(pWeights, pWeights+sizeof pWeights/sizeof pWeights[0]);
            dimensions_ = dimensions;
        }
        static LinearFeatureResponsePatchesSVM CreateRandom(Random& random, const IDataPointCollection& data, unsigned int* dataIndices, const unsigned int i0, const unsigned int i1,float svm_c, FeatureMaskType featureMask, bool root_node);
        static void GenerateMaskPatches(Random& random, std::vector<int>& vIndex, int dims , bool root_node);


        float GetResponse(const IDataPointCollection &data, unsigned int index) const;
        std::string ToString()  const;

        static void Serialize(LinearFeatureResponsePatchesSVM lr, std::ostream& o)
        {
            o.write((const char*)(&lr.dimensions_), sizeof(lr.dimensions_));
            o.write((const char*)(&lr.nWeights_), sizeof(lr.nWeights_));
            o.write((const char*)(&lr.vIndex_), sizeof(int)*lr.vIndex_.size());
        ...
        static LinearFeatureResponsePatchesSVM Deserialize(std::istream& i)
        {
